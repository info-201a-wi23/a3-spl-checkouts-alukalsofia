...
#What is the average and the total number of checkouts for each Material Type? 
...
book_plot <- ggplot(data = book_six_years) +
  geom_point(aes(x = CheckoutMonth, y = book_sum, color = CheckoutYear)) +
...
ebook_plot <- ggplot(data = ebook_six_years) +
  geom_point(aes(x = CheckoutMonth, y = ebook_sum, color = CheckoutYear)) +
